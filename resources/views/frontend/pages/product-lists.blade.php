...
@section('title', 'E-TECH || PRODUCT PAGE')
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            /*  Jquery Ui slider js
            /*----------------------------------------------------*/
            if (Rp("#slider-range").length > 0) {
                const max_value = parseInt(Rp("#slider-range").data('max')) || 500;
                const min_value = parseInt(Rp("#slider-range").data('min')) || 0;
                const currency = Rp("#slider-range").data('currency') || '';
                let price_range = min_value + '-' + max_value;
                if (Rp("#price_range").length > 0 && Rp("#price_range").val()) {
                    price_range = Rp("#price_range").val().trim();
                }

                let price = price_range.split('-');
                Rp("#slider-range").slider({
                    range: true,
                    min: min_value,
                    max: max_value,
                    values: price,
                    slide: function(event, ui) {
                        Rp("#amount").val(currency + ui.values[0] + " -  " + currency + ui.values[1]);
                        Rp("#price_range").val(ui.values[0] + "-" + ui.values[1]);
                    }
                });
            }
            if (Rp("#amount").length > 0) {
                const m_currency = Rp("#slider-range").data('currency') || '';
                Rp("#amount").val(m_currency + Rp("#slider-range").slider("values", 0) +
                    "  -  " + m_currency + Rp("#slider-range").slider("values", 1));
            }
        })
    </script>
@endpush
